...
const registerUser = async (req, res) => {
    ...
    if (existedUser) {
        return res.status(409).json({ status: 409, message: "User with username already exists" })
    }
    try {
        ...
        if (!createdUser) {
            return res.status(500).json({ status: 500, message: "Something went wrong while registering the user" })
        }
...
const loginUser = async (req, res) => {
    ...
    if (!username || !password) {
        return res.status(400).json({ status: 400, message: "username and passsword is required" });
    }
    ...
    const loggedInUser = await User.findById(user._id).select("username email fullName friends");
...
const searchUsers = async (req, res) => {
    ...
    if (!(typeof q === "string" && q.length > 0)) return res.status(200).send({ users: [], message: "Please enter words." });
    try {
