...
function Chat() {
    ...
    function sendMessage(e: FormEvent) {
        ...
        const msg: Message = { sender: username, receiver, type: "text", content: currentInput, time: new Date().toLocaleString("en-US", { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit' }) };
    ...
    return (
        <main className="h-screen flex items-center bg-neutral-800 text-white">
            <div className={`bg-neutral-900 h-full z-40 w-full sm:w-96 ${receiver ? "hidden sm:flex" : "flex"}`}>
                <Sidebar />
                <div className="w-full overflow-y-scroll flex flex-col p-3 h-full">
                    ...
                    <div className=" flex-1">
                        {friends.length === 0 && (
                            <p className="text-center mt-5 text-gray-500">Friends not Available.</p>
                        )}
            ...
            <div className={`flex-grow flex-col h-full ${receiver ? "flex" : "hidden sm:flex"}`}>
                {receiver ? (
                    <>
                        <div className="flex justify-between items-center bg-zinc-900">
                            <div className="ml-2 flex items-center">
                                <button onClick={() => { navigate(-1) }} className="font-bold text-2xl sm:hidden"><ArrowLeft height={30} width={40} /></button>
                                <span className="text-3xl p-2 font-semibold">{receiver}</span>
                            </div>
                            <span className="text-3xl p-2 font-semibold mr-2">{username}</span>
                        </div>
                        ...
                        <div className="m-auto rounded-lg overflow-hidden bg-white pl-2 my-3 font-normal w-80 sm:w-96 md:w-1/2 md:min-w-fit">
                            <form className="flex items-center" onSubmit={(e) => { sendMessage(e) }}>
                                <input className="outline-none bg-transparent p-3 text-black flex-grow" type="text" placeholder="Enter Message" value={currentInput} onChange={(e) => { setCurrentInput(e.target.value) }} autoComplete="off" spellCheck="false" autoFocus={true} />
                                <button className={`text-black mx-2 hidden sm:block ${isListening && "text-orange-500"}`} onClick={startListening} hidden={!micSupported}>
                                    <MicIcon width={30} height={30} />
                                </button>
                                <button className="bg-orange-400 px-3 py-3 hidden sm:block" type="submit">Send</button>
                            </form>
                        </div>
                    </>
                ) : (
                    <h2 className="text-center mt-16 text-gray-500 text-4xl">No User Selected</h2>
                )}
            </div>
        </main>
    )
